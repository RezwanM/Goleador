...
//Global namespace
var affdex = affdex || {};
affdex.version="3.4.1d.69-d1ee853"
...
affdex.Detector.prototype.start = function() {
  if (!this.isRunning) {
    XHRWorker(this.assets["affdex-worker.js"], function(worker) {
       this.worker = worker;
       this.worker.onmessage = this.onWorkerMessage;
       this.worker.postMessage( { "message": "ctor", "assets": this.assets} );
       this.isRunning = true;
    }, this);
  }
  else {
...
affdex.CameraDetector = function(element, imgW, imgH, faceMode) {
  ...
  var captureImage = function() {
    if (self.isWorkerInitialized && canvasElement) {
      ...
      canvasContext.drawImage(self.videoElement, 0, 0, self.videoElement.videoHeight, self.videoElement.videoHeight);
      var imgData = canvasContext.getImageData(0, 0, canvasElement.width, canvasElement.height);
      var currentTimeStamp = (new Date()).getTime() / 1000;
      console.log("captureImage at " + currentTimeStamp);
      process(imgData, currentTimeStamp - startTimeStamp);
    }
  };

  self.captureNextImage = function() {
      captureImage();
  }
